...
class BrandService {
  ...
  async createBrand(data, userId) {
    ...
    // Sync: set brand_id on assigned items
  ...
  async updateBrand(brandId, userId, updateData) {
    ...
    if (item_ids !== undefined) {
      ...
      // Set brand_id on newly added items
      if (addedIds.length > 0) {
        // Also remove these items from any other brand's item_ids
