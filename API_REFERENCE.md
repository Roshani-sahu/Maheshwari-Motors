...
| 3 | POST | `/auth/register` | Register new user (deprecated — use admin flow) |
| 4 | POST | `/auth/login` | User login (deprecated — use admin/firm login) |
| 5 | POST | `/auth/admin/register` | Create a new admin account (admin-only) |
...
```json
{
  "party_id": "...",
  "items": [
    { "item_id": "...", "quantity": 5, "rate": 1500, "is_gst": 1 }
  ]
}
```
