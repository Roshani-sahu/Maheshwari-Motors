...
import { FaPrint, FaDownload, FaEye, FaTimes, FaBarcode } from 'react-icons/fa';
import { useApp } from '../contexts/AppContext';
import { formatCurrency, formatDate, printDocument } from '../utils';

const PrintPreview = ({ 
  isOpen, 
  onClose, 
  documentType = 'challan', // challan, bill, invoice
  documentData,
  className = ""
}) => {
  const { state } = useApp();
  ...
  const renderFirmHeader = () => {
    if (printSettings.firmFormat === 'minimal') {
      return (
        <div className="text-center mb-4">
          <h2 className="text-lg font-bold">{selectedFirm?.name}</h2>
        </div>
      );
    }
    ...
    return (
      <div className="print-header">
        <h1 className="text-2xl font-bold mb-2">{selectedFirm?.name}</h1>
        <div className="text-sm">
          <p>123 Business Street, Industrial Area</p>
          <p>City - 123456, State</p>
          <p>Phone: +91 98765 43210 | Email: info@{selectedFirm?.name.toLowerCase().replace(' ', '')}.com</p>
